...
from rle_assignment.utils import LinearSchedule, RingBuffer, PrioritizedRingBuffer
...
def compute_modified_reward(self, obs, action):
    shot_down_enemy = self.detect_shot_down_enemy(obs, action)
    avoided_bullet = self.detect_avoided_bullet(obs, action)
    avoided_bullet_behind_block = self.detect_avoided_bullet_behind_block(obs, action)
    destroyed_block = self.detect_destroyed_block(obs, action)

    reward_modification = 0
    if shot_down_enemy:
        reward_modification += 0.1
    if avoided_bullet:
        reward_modification += 0.1
    if avoided_bullet_behind_block:
        reward_modification += 0.1
    if destroyed_block:
        reward_modification -= 0.3
...
def train(device):
    ...
    replay_buffer = PrioritizedRingBuffer(size=FLAGS.buffer_size, specs={
    ...
    for global_step in range(FLAGS.total_steps):
        ...
        if global_step > FLAGS.warmup_steps and global_step % FLAGS.train_freq == 0:
            ...
            batch, indices, weights = replay_buffer.sample(FLAGS.batch_size)
            ...
            with torch.no_grad():
                td_errors = td_target - selected_q_values.squeeze()
