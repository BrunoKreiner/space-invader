...
The inidial idea behind DQN is to combine deep learning with reinforcement learning. Traditional Q-Learning struggles with high dimensional state spaces, a problem deep learning can help overcome.
...
- Episode Frames and Episode Steps:
    - These metrics show the amount of frames in an episode as well as the amount of steps the agent takes. These are inherently correlated. In gym's SpaceInvader environment there is a parameter called frameskipping which sets the amount of frames passing by for each action. In the DQN trained, the amount of episode steps correlated to episode frames divided by 3. Looking at the episode frames we can see whether the agent is surviving longer. 
- FPS:
...
- Videos: 
    - Videos provide qualitative assessment of the agent's behavior and strategy. How does the agent act in one episode?
...
We can see that the agent doesn't learn and the metrics obsiously look stable. The episode_reward is basically an average reward when every aciton is random. The FPS are very pretty high on my local system since no neural network is trained during the steps. If we look at the game recorded:
...
In standard DQN, the same network (Q-network) is used to both select and evaluate an action. This can lead to an overoptimistic estimate of the Q-value, as the network might consistently overestimate the Q-value of certain actions due to natural occuring noise. The main idea behind Double DQN is to reduce this overestimation bias that can occur. It arises due to the max operator used in the target network's "next q values" estimation, which selects the maximum value from a noisy estimate of several action values. 
...
In Double DQN, the issue is handled by using the Q-network to select the best action of the next observation to then calculate the next q values using the target network and the indeces of the best actions chosen by the Q-network. This makes the "next q-values" calculation use both networks at the same time reducing noise and the overestimation bias. 
...
Looking at the [Train Logs](https://github.com/BrunoKreiner/space-invader/runs/double_dqn/train_logs.txt), the reward after the same amount of steps as in the simple DQN example wasn't much higher but due to large fluctuations, some of the episodes reached over 300 points as soon as the 860'000th step which is remarkable and not seen in the simple DQN results. We can see the epsilon decreasing making the model exploit more down the line instead of explore. The loss seems to be going up over time which is weird since it's trying to minimize the huber loss. An increasing loss might indicate lots of things. The increase of the loss shows that the model is not finished with learning. It is also not the best metric for performance since there are a lot of different states to be looked at during training that might make it hard during training to correctly predict the sum of rewards by the Q-network. The Q-value is increasing steadily which is a good sign and shows the model's performance in predicting higher future rewards for its actions. 
